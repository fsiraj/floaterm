...
M.cycle_term_bufs = function(direction)
   if not state.terminals or #state.terminals == 0 then return end
...
M.delete_term = function(buf)
   ...
   if not buf then
      ...
      if i then buf = state.terminals[i].buf end
   end
   ...
   if buf then
      ...
      if method == 'manual' then vim.api.nvim_buf_delete(buf, { force = true }) end
...
M.send_cmd = function(opts)
   if not state.terminals then
      ...
      if opts.name then bufdetails = utils.get_term_by_key(opts.name, 'name')[2] end

      local job_id = vim.b[bufdetails.buf].terminal_job_id
      vim.api.nvim_chan_send(job_id, opts.cmd .. ' \n')
      vim.api.nvim_buf_call(bufdetails.buf, function() vim.cmd([[normal G]]) end)
   end
end
